...
int validarData_nascimento(TJogador **jogadores, int *qtd_jogadores);
int validarData_partida(TPartida **partidas, int *qtd_partidas);
void validarHora(TPartida **partidas, int *qtd_partidas);
void cadastrarJogador(TJogador **jogadores, int *qtd_jogadores);
void cadastrarPartidas(TJogador **jogadores, TPartida **partidas, int *qtd_jogadores, int *qtd_partidas);
void listarJogadores(TJogador *jogadores, int *qtd_jogadores);
void listarPartidas(TPartida *partidas, int *qtd_partidas);
void salvarJogador(TJogador **jogadores, int *qtd_jogadores);
...
void opcaoMenu(int opcao) 
{
    ...
    switch(opcao) {
        case 1:
            cadastrarPartidas(&jogadores, &partidas, &qtd_jogadores, &qtd_partidas);
            break;
        case 2: 
...
int validarData_nascimento(TJogador **jogadores, int *qtd_jogadores)
{
    ...
    fflush(stdin);
    int d = (*jogadores)[*qtd_jogadores].data_nascimento.dia;
    int m = (*jogadores)[*qtd_jogadores].data_nascimento.mes;
    int a = (*jogadores)[*qtd_jogadores].data_nascimento.ano;
...
{
    CLS   
    int d = (*partidas)[*qtd_partidas].data_partida.dia;
    int m = (*partidas)[*qtd_partidas].data_partida.mes;
    int a = (*partidas)[*qtd_partidas].data_partida.ano;
...
void validarHora(TPartida **partidas, int *qtd_partidas)
{
    ...
    do 
    {
        ...
        int m = (*partidas)[*qtd_partidas].hora_partida.minuto;

        hora_valida = 1;
        if (h < 0 || h > 23) 
        {
            hora_valida = 0;
            printf("Hora invalida! Deve ser entre 00 e 23. ");
        }
        if (m < 0 || m > 59) 
        {
            hora_valida = 0;
            printf("Minuto invalido! Deve ser entre 00 e 59. ");
        }
...
void cadastrarJogador(TJogador **jogadores, int *qtd_jogadores) 
{
    while(1)
    {
        ...
        validarData_nascimento(jogadores, qtd_jogadores);
...
void cadastrarPartidas(TJogador **jogadores, TPartida **partidas, int *qtd_jogadores, int *qtd_partidas)
{
    ...
    scanf("%d%d%d", &(*partidas)[*qtd_partidas].data_partida.dia,
                    &(*partidas)[*qtd_partidas].data_partida.mes, 
                    &(*partidas)[*qtd_partidas].data_partida.ano);
    fflush(stdin);
    }
    while(validarData_partida(&partidas, &*qtd_partidas));

    validarHora(partidas, qtd_partidas);
    ...
    {
        (*partidas)[*qtd_partidas].jogadores = (TJogador*)malloc(maxJogadores * sizeof(TJogador));
...
void salvarJogador(TJogador **jogadores, int *qtd_jogadores)
{
    ...
    {
        fprintf(pArq,"%s;",jogadores[i]->nome_jogador);
        fprintf(pArq,"%s;",jogadores[i]->data_nascimento);
    }
}